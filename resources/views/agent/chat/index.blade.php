        ...
        <div class="lg:col-span-1">
            <div class="conversations-list h-full">
                <div class="p-4 border-b border-white/5">
                    ...
                    <p class="text-gray-400 text-sm mt-1">Mazungumzo na wateja</p>
                </div>
                
                <div class="overflow-y-auto" style="max-height: calc(100vh - 300px);">
                    @forelse($conversations ?? [] as $conversation)
                        <a href="{{ route('agent.chat.show', $conversation['line_request_id']) }}" 
                           class="conversation-item flex items-center gap-4 {{ request()->route('lineRequest')?->id == $conversation['line_request_id'] ? 'active' : '' }}">
                            <div class="conversation-avatar">
                                {{ substr($conversation['participant']['name'] ?? 'C', 0, 1) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between">
                                    <span class="font-bold text-white truncate">{{ $conversation['participant']['name'] ?? 'Customer' }}</span>
            ...
            <div class="chat-window">
                @if(isset($selectedRequest))
                    ...
                    <div class="chat-header">
                        <div class="conversation-avatar">
                            {{ substr($participant['name'] ?? 'C', 0, 1) }}
                        </div>
                        <div class="flex-1">
                            <h3 class="font-bold text-white">{{ $participant['name'] ?? 'Customer' }}</h3>
                            <p class="text-sm text-gray-400">{{ $participant['phone'] ?? '' }}</p>
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            <!-- Directions Button -->
                            @if($selectedRequest->customer_latitude && $selectedRequest->customer_longitude)
                                <a href="https://www.google.com/maps/dir/?api=1&destination={{ $selectedRequest->customer_latitude }},{{ $selectedRequest->customer_longitude }}&travelmode=driving" 
                                   target="_blank" 
                                   class="directions-btn">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"></path>
                                    </svg>
                                    {{ __('messages.map.get_directions') }}
                                </a>
                            @endif
                        </div>
                    </div>
                        ...
                        @empty
                            <div class="flex-1 flex items-center justify-center">
                                <div class="text-center">
                                    ...
                                    <p class="text-gray-600 text-sm mt-1">Anza mazungumzo na mteja!</p>
                                </div>
                            </div>
                        @endforelse
                    </div>
                    ...
                    <!-- Chat Input -->
                    <form action="{{ route('agent.chat.store', $selectedRequest) }}" method="POST" class="chat-input-container" id="chatForm">
                        @csrf
                ...
                @else
                    <div class="empty-state">
                        ...
                        <p class="text-gray-400 max-w-sm">Chagua mazungumzo kutoka orodha kushoto kuwasiliana na mteja.</p>
                    </div>
                @endif
            </div>
        </div>
